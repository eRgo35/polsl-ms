...
dane <- read.csv("D:\\RomanowOLF\\polsl-ms\\baza-MS.csv")
...
# Funkcja do implementacji procedury Romano-Wolfa metodą stepdown
stepdown_romano_wolf <- function(Y, X, alpha, M) {
  ...
  list(R = R, sorted_indices = sorted_indices, sorted_stats = sorted_stats)  # zwrócenie wyników
}
...
# Wyświetlenie nieodrzuconych hipotez z tytułem kolumny i wartością statystyki
if (result$R < length(result$sorted_indices)) {
  ...
  for (i in 1:length(not_rejected_hypotheses)) {
    cat("Hipoteza:", not_rejected_hypotheses[i], "Statystyka:", not_rejected_stats[i], "\n")
  }
}
